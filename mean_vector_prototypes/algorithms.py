...
class ProtoSim(Algorithm):
    def __init__(self, num_classes, cfg, proto_path):
        super(ProtoSim, self).__init__(num_classes, cfg)
        self.network = models.get_net(cfg)
        self.classifier = models.get_classifier(self.network.out_features(), cfg)
        self.sim_loss_criterion = Sim_Loss(cfg, self.network.out_features(), proto_path)

        model_updates = [{"params":self.network.parameters()},
                         {"params":self.classifier.parameters()},
                         {"params":self.sim_loss_criterion.parameters(), "lr": cfg.LEARNING_RATE * cfg.PROTO_LR_MULTI},
                        ]
    ...
    def update(self, minibatch, cfg):
        ...
        if self.epoch + 1 <= cfg.WARM_UP_EPOCHS:
            ...
            sim_loss = self.sim_loss_criterion(all_features, domain_batch, label_batch, cfg)
            total_loss = ce_loss + cfg.SIM_ALPHA * sim_loss
    ...
    def validate(self, val_loader, test_loader, writer):
        ...
        test_auc = -1
        if self.epoch <= self.cfg.EPOCHS:
            val_auc, val_loss, _, _ = algorithm_validate(self, val_loader, writer, self.epoch, 'val')
            test_auc, test_loss, test_acc, test_f1 = algorithm_validate(self, test_loader, writer, self.epoch, 'test')
            if self.epoch == self.cfg.EPOCHS:
                self.epoch += 1
        else:
            test_auc, test_loss, test_acc, test_f1 = algorithm_validate(self, test_loader, writer, self.cfg.EPOCHS + self.cfg.VAL_EPOCH, 'test')
            logging.info('Best performance on test domain(s): {}'.format(test_auc))
