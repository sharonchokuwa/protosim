...
class Sim_Loss(nn.Module):
    ...
    def forward(self, feat, domain_batch, label_batch, cfg, prototypes):

        prots_a = prototypes[domain_batch, label_batch]  # Shape: [batch_size, 2048]

        # Calculate similarities between samples and prototypes
        sim_a = self.calculate_sim(feat, prots_a, cfg, dim=1)  # Shape: [batch_size]
        all_prototypes = prototypes.clone()
