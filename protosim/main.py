...
def run_algo(target_domain, source_domains, cfg, timestamp, dataset_num, excel_data, args):
    ...
    proto_path = os.path.join('artifacts', 'init_saved_prototypes', target_domain[0], 'init_prototypes.pkl')
    ...
    scheduler = get_scheduler(algorithm.optimizer[0], num_epochs)
    ...
    for i in iterator:
        ...
        loss_dc_avg = LossCounter()
        loss_scaled_dc_avg = LossCounter()
        loss_sim_avg = LossCounter()
        loss_scaled_sim_avg = LossCounter()
        ...
        for image, mask, label, domain, img_index in train_loader:
            ...
            loss_dc_avg.update(loss_dict_iter['domain_center_loss'])
            loss_scaled_dc_avg.update(loss_dict_iter['scaled_center_loss'])
            loss_sim_avg.update(loss_dict_iter['sim_loss'])
            loss_scaled_sim_avg.update(loss_dict_iter['scaled_sim_loss'])

        update_writer_ours(writer, epoch, loss_avg, loss_ce_avg, loss_dc_avg, \
                                         loss_scaled_dc_avg, loss_sim_avg, loss_scaled_sim_avg,)                                
        ...
        if epoch % cfg.VAL_EPOCH == 0:
            ...
            prototypes = algorithm.center_loss.get_centers()
    ...
    if cfg.SAVE_PROTOTYPES:
        algorithm.center_loss.save_prototypes_info(cfg, epoch)
